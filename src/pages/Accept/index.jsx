...
export default function Accept() {
  ...
  const countPassengers = (way) => {
    ...
    for (let i = 0; i < seats[way].length; i += 1) {
      ...
      if (newPassenger.is_child === true)
        newPassenger.include_children_seat = false;
      ...
      if (passengers[i].person.type === "child")
        newPassenger.person_info.is_adult = false;
      if (passengers[i].person.gender === "female")
        newPassenger.person_info.gender = false;
      if (passengers[i].doc.docType === "passport")
        newPassenger.person_info.document_data =
          passengers[i].doc.series + passengers[i].doc.number;
    ...
    <section className="table flex">
      ...
      <div className="column offers">
        ...
        <div className="btn__wrapper">
          <button className="button standart__white" onClick={confirmOrder}>
            Подтвердить
          </button>
        </div>
      </div>
