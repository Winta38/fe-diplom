...
export default function Paths() {
  ...
  const submitParams = (e) => {
    ...
    if (e.key === "Enter") {
      ...
      departDate.codedDate
        ? (newParamsArr[2] = `date_start=${departDate.codedDate}`)
        : (newParamsArr[2] = "");
      arriveDate.codedDate
        ? (newParamsArr[3] = `date_end=${arriveDate.codedDate}`)
        : (newParamsArr[3] = "");
      const newParams = {
        pathname: "/routes",
    ...
    <aside className="parameters parameters__path">
      <form className="form__side" onKeyDown={submitParams} id="side_calendar">
        ...
        <div className="inputWrapper inputWrapper__side">
          ...
          ></input>
          <span
            className="clearup__icon__side lightgrey"
            id="depart"
            onClick={clearInput}
          >
            &#10006;
