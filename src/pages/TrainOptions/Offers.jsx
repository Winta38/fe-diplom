...
export default function Offers({ data, back }) {
  ...
  useEffect(() => {
    filters.forEach((value) => {
      itemRange = itemRange.filter(
        (item) => item.route.departure[value] === true
      );
    });

    itemRange = itemRange.filter(
      (item) =>
        priceRange.min <= item.minPrice && priceRange.max >= item.maxPrice
    );

    itemRange = itemRange.filter(
      (item) =>
        timeRange.from <= item.timeRange.from &&
        timeRange.to >= item.timeRange.to
    );

    if (back)
      itemRange = itemRange.filter(
        (item) =>
          timeRangeBack.from <= item.timeRangeBack.from &&
          timeRangeBack.to >= item.timeRangeBack.to
      );
  ...
  useEffect(() => {
    ...
    switch (type) {
      case "time":
        newArr.sort(
          (a, b) =>
            a.route.departure.from.datetime - b.route.departure.from.datetime
        );
        break;
      case "price":
        newArr.sort((a, b) => a.minPrice - b.minPrice);
        break;
      case "duration":
        newArr.sort(
          (a, b) => a.route.departure.duration - b.route.departure.duration
        );
    }
    setItems([...newArr]);
  }, [type]);
    ...
    <div className="offers offers__trains">
      ...
      {items.length > 0 ? (
        <List className="offers__list">
          {items.map((item) => (
            <Train key={items.indexOf(item)} item={item.route} />
          ))}
        </List>
      ) : (
