  ...
  return (
    <div className="offers__header flex">
      ...
      <div className="filters flex">
        <label className="select__time flex px16 grey">
          сортировать по:
          <div className="customSelect black" onClick={dropdown}>
            <p className="black">{type.name}</p>
            {open && (
              <List className="customDropdown">
                {types.map((item) => (
                  <li key={item.id} id={item.id} onClick={handleChange}>
                    {item.name}
                  </li>
                ))}
              </List>
        ...
        <label className="select__number flex px16 grey">
          показывать по:
          {filters.map((item) => (
            <p className="px16" key={item} id={item}>
              {item}
            </p>
          ))}
        </label>
