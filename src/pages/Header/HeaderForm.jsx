...
export function HeaderForm({ loc, callPopup }) {
  ...
  const showDate = (dateMillisec, date, name, codedDate) => {
    name === "depart" && setDepartDate({ dateMillisec, date, codedDate });
    setCalendarTo(false);
    name === "arrive" && setArriveDate({ dateMillisec, date, codedDate });
    setCalendarBack(false);
  };
  ...
  return (
    <form
      onSubmit={submitForm}
      className={`headerForm flex__column ${
        loc === "/" ? "headerForm__normal" : "headerForm__slim"
      }`}
    >
      <div
        className={`forwWrapper ${
          loc === "/" ? "forwWrapper__normal" : "formWrapper__slim flex"
        }`}
      >
        <div className="row__first">
          ...
          <div className="form__row flex__standart">
            <div className="inputWrapper">
              ...
              ></input>
              {focusFrom && inputFrom.name && cities.data.length > 0 && (
                <div className="cityOptions">
                  {cities.data.map((city) => (
                    <p
                      className="city"
                      key={city._id}
                      onClick={() => fillFromList(city, "from")}
                    >
                      {city.name}
                    </p>
                  ))}
            ...
            <div className="inputWrapper">
              ...
              ></input>
              {focusTo && inputTo.name && cities.data.length > 0 && (
                <div className="cityOptions">
                  {cities.data.map((city) => (
                    <p
                      className="capital"
                      key={city._id}
                      onClick={() => fillFromList(city, "to")}
                    >
                      {city.name}
                    </p>
                  ))}
        ...
        <div className="row__second">
          ...
          <div
            className={`form__row flex__standart ${
              loc === "/" ? "" : "form__row__slim"
            }`}
          >
            <div className="inputWrapper">
              ...
              ></input>
              <span
                className="clearup__icon lightgrey"
                id="depart"
                onClick={clearInput}
              >
                &#10006;
              ...
              {calendarTo && (
                <Calendar
                  chosenDate={departDate.dateMillisec}
                  showDate={showDate}
                  name="depart"
                />
              )}
            ...
            <div className="inputWrapper">
              ...
              {calendarBack && (
                <Calendar
                  chosenDate={arriveDate.dateMillisec}
                  showDate={showDate}
                  name="arrive"
                />
              )}
      ...
      <button type="submit" className="button standart form__submit medium">
        Найти билеты
      </button>
    </form>
  );
