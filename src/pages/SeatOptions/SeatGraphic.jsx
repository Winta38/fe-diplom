...
export default function SeatGraphic({ coach, seats, back, category }) {
  ...
  const seatNumbers = useSelector(
    (state) => state.root.trainSettings.seatNumbers
  );
  const {
    highSeats,
    lowSeats,
  ...
  const getPrice = (costType) => {
    ...
    switch (costType) {
      case "top_seat":
        result = coach.top_price;
        break;
      case "bottom_seat":
        result = coach.bottom_price;
        break;
    }
    return result;
  };
  ...
  const gatherProps = (item) => {
    const props = {
      ...
      alreadyChosen: chosenByDirection.find(
        (e) => e.item === item && e.vagon === coach.name
      ),
      chooseSeat,
    };
    return props;
  ...
  return (
    <div className={`seats ${type === "fourth" && "smallSeats"}`}>
      <Div className="seatRow flex__standart">
        {type !== "first"
          ? highSeats.map((item) => (
              <Seat props={gatherProps(item)} key={item} />
            ))
          : firstClassSeats.map((item) => (
              <Seat props={gatherProps(item)} key={item} />
            ))}
      </Div>
      ...
      {type === "third" || type === "fourth" ? (
        <Div
          className={`seatRow flex__standart ${
            type === "fourth" ? "thinRow" : "sideRow"
          }`}
        >
          {type === "third" &&
            sideSeats.map((item) => (
              <Seat props={gatherProps(item)} key={item} thirdRow={true} />
            ))}
          {type === "fourth" &&
            sittingSeats.map((item) => (
              <Seat props={gatherProps(item)} key={item} />
            ))}
        </Div>
      ) : (
      ...
      {type === "fourth" && (
        <Div className="seatRow flex__standart">
          {remainingSittingSeats.map((item) => (
            <Seat props={gatherProps(item)} key={item} />
          ))}
        </Div>
      )}
