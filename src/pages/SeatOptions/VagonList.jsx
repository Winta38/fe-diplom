...
export default function VagonList({ seats, back, category }) {
  const choosingWithYou = useSelector(
    (state) => state.root.trainSettings.choosingWithYou
  );
    ...
    <div className="vagonList">
      <div className="vagonList__header flex__standart">
        <Div className="vagon__numbers flex__standart">
          ...
          {seats.map((item) => (
            <p key={item.coach.name} className="bold px18">
              {item.coach.name}
            </p>
          ))}
        </Div>
      ...
      {seats.map((item) => (
        <VagonUnit
          ...
          vagon={item}
          choosingWithYou={choosingWithYou}
          back={back}
          category={category}
        />
      ))}
      {sum > 0 && (
        <div className="ticketCost flex__end">
