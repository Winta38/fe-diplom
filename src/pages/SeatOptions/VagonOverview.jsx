...
export default function VagonOverview({ vagonTypes, seats, back, category }) {
  ...
  return (
    <>
        ...
        <List className="type__list flex">
          {vagonTypes.map(
            (item) =>
              item.status && (
                <div
                  key={vagonTypes.indexOf(item)}
                  className="seat__type__comfort"
                  onClick={() => showVagon(item.className)}
                >
                  <span
                    className={`type__icon greyIcon ${item.className} ${
                      vagonType === item.className ? "icon__active" : ""
                    }`}
                  ></span>
      ...
      {vagonOpen && (
        <VagonList
          seats={seats.filter((item) => item.coach.class_type === vagonType)}
          back={back}
          category={category}
        />
      )}
