...
export default function Passengers() {
  ...
  const passengerCount = Math.max(
    ...Object.values(seats).map((item) => item.length)
  );
  const dispatch = useDispatch();
  const items = Array(passengerCount)
    .fill()
    .map(() => ({
    ...
    <section className="table flex">
      ...
      <div className="column offers">
        <List className="offers__passengers">
          {passengerArr.map((item) => (
            <PassengerChoice
              key={item.id}
              id={item.id}
              num={passengerArr.indexOf(item) + 1}
