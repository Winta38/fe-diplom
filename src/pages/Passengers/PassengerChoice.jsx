...
export default function PassengerChoice({ id, num, deletePassenger }) {
  ...
  const checkDate = (birthdate) => {
    const err = new Error(
      "Дата рождения введена некорректно! Пример: 10.05.1992",
      { cause: "birthdate" }
    );
  ...
  const checkSeries = (series) => {
    if (series.match(/[0-9]{4}/g) === null)
      throw new Error("Серия паспорта введена некорректно! Пример: 1234", {
        cause: "series",
      });
  };
  ...
  const checkNumber = (number) => {
    if (number.match(/[0-9]{6}/g) === null)
      throw new Error("Номер паспорта введен некорректно! Пример: 123456", {
        cause: "number",
      });
  };
  ...
  const checkBirthCert = (number) => {
    const result = number.match(
      /^(I|II|III|IV|V|VI|VII|VIII|IX|X)-[А-Я]{2}-[0-9]{6}$/g
    );
    if (result === null)
      throw new Error(
        "Номер свидетельства о рождении введен некорректно! Пример: VI-МЯ-193452",
        { cause: "number" }
      );
  };
  ...
  return (
    <form
      className="panel__wrapper psg"
      id={`totalPassengerForm${num}`}
    ...
    >
      <div className="panel__standart border__btm flex__standart">
        <div className="flex__standart">
          <button
            type="button"
            className={`${
              open ? "button__round" : "button__round__orange"
            } flex__center`}
            onClick={openWindow}
          >
            {open ? "—" : "+"}
      ...
      <div className={!open ? "minimized" : ""}>
        ...
        <div
          className={`form__wrapper__standart ${
            errLength > 0 && "errorWindow"
          } ${complete && "completeWindow"} ${
            errLength > 0 || complete ? "flex__standart" : "flex__end"
          }`}
        >
          {complete && (
          ...
          {errLength > 0 && (
            <div className="popup__message flex__standart">
              <span
                className="icon__error flex__center px18"
                onClick={acceptError}
              >
                &#x2716;
              </span>
