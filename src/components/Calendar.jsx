...
export default function Calendar({ chosenDate, showDate, name }) {
  ...
  const callback = (day) => {
    ...
    showDate(
      Date.parse(newDate),
      format(newDate, "dd.LL.yyyy"),
      name,
      format(newDate, "yyyy-LL-dd")
    );
  };
    ...
    <div className="calendarWrapper">
      ...
      <div className="calendar">
        <div className="calendarHeader flex__standart">
          <p onClick={prevMonth} className="changeMonth">
            &#129168;
          </p>
          <p className="month">{format(currentDate, "LLLL", { locale: ru })}</p>
          <p onClick={nextMonth} className="changeMonth">
            &#129170;
          </p>
        </div>
        <div className="calendarGrid">
          {Array.from({ length: dayOfTheWeek - 1 }).map((_, day) => (
            <Cell key={day} />
          ))}
          ...
          {Array.from({ length: fullMonth }).map((_, day) => {
            ...
            return (
              <Cell
                key={correctDay}
                passed={(correctDay < today && equalMonth) || passedMonth}
                active={chosenDay === correctDay}
                prop={correctDay}
                callback={callback}
              />
            );
          ...
          {daysAfterEnd !== 7 &&
            Array.from({ length: daysAfterEnd }).map((_, day) => (
              <Cell key={day} />
            ))}
        </div>
      </div>
