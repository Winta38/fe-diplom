...
const searchParams =
  localStorage.getItem("searchParams") != null
...
const route =
  localStorage.getItem("route") != null
    ...
    : {};
const seats_to =
  localStorage.getItem("seats_to") != null
    ? JSON.parse(localStorage.getItem("seats_to"))
    : [];
const seats_back =
  localStorage.getItem("seats_back") != null
    ? JSON.parse(localStorage.getItem("seats_back"))
    : [];
const services_to =
  localStorage.getItem("services_to") != null
    ? JSON.parse(localStorage.getItem("services_to"))
    : [];
const services_back =
  localStorage.getItem("services_back") != null
    ? JSON.parse(localStorage.getItem("services_back"))
    : [];
const passengers =
  localStorage.getItem("passengers") != null
...
const order = createSlice({
  ...
  reducers: {
    ...
    addRoute(state, action) {
      ...
      state.id =
        generateID("0123456789", 3) +
        generateID("ABCDEFGHIJKLMNOPQRSTUVWXYZ", 2);
    ...
    changeSeats(state, action) {
      ...
      if (action.payload.back) {
        const found = state.places.back.find(
          (e) => e.item === data.item && e.vagon === data.vagon
        );
        const category = state.seatsBack.find(
          (e) => e.codename === data.category
        );
      ...
      const found = state.places.to.find(
        (e) => e.item === data.item && e.vagon === data.vagon
      );
    ...
    addPassenger(state, action) {
      const found = state.passengers.find(
        (e) => e.id === action.payload.formData.id
      );
